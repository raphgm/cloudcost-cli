-- Real check: Azure Database for PostgreSQL Flexible Server instances
-- with sustained low CPU -- the provisioned compute tier bills hourly
-- regardless of connections or query load. Price fetched live per SKU
-- via the Retail Prices API.
SELECT
    'azure' AS provider,
    resource_id,
    'Azure Database for PostgreSQL' AS service_name,
...
FROM fact_postgres_idle_flexible
